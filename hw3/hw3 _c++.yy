...
%token BREAK CHAR CASE CONTINUE DECLARE DO IF EXIT FOR FUNCTION LOOP PRINT PRINTLN RETURN VAR REAL ARRAY OF END PROCEDURE CONST CBEGIN PUT GET RESULT WHEN SKIP THEN DEFAULT DECREASING IN
%token <token> TRUE
%token <token> FALSE
...
condition: IF expression IF_PREACT THEN statements END IF %prec LOWER_THAN_ELSE  {	
...
condition_r: IF expression IF_PREACT THEN statements END IF %prec LOWER_THAN_ELSE  {	
			if($2.type != TokenType::vbool){
				//Error("Expression must be boolean");
			}
			fprintf(output, "L%d:\n", $<token>2._int-1);
